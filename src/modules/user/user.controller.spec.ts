...
describe('User Controller', () => {
  ...
  it('Usuario debe poder cambiar contraseña con token', () => {
    const req = mocks.createRequest();
    req.res = mocks.createResponse();
    const resetPassDto = {
      password: '123456',
    };
    expect(userController.resetPassword(req, resetPassDto)).toEqual(
      'Contraseña a sido cambiada',
    );
    expect(mockUserService.resetPassword).toHaveBeenCalled();
  });
  it('Usuario debe poder iniciar sesion', () => {
    const res = mocks.createResponse();
    const loginDto = {
      us_correo: 'kjor29@gmail.com',
      password: '123456',
    };
    expect(userController.login(loginDto, res)).toEqual(
      'Inicio de sesión exitoso',
    );
    expect(mockUserService.login).toHaveBeenCalled();
  });
  it('Usuario debe poder solicitar access token', () => {
    const req = mocks.createRequest();
    req.res = mocks.createResponse();
